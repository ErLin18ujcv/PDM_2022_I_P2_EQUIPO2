...
class ActivityClientes : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nuevoClienteBtn.setOnClickListener {
            ingresarCliente()
        }

        //llenar list view
        var array = ArrayList<Cliente>()
        for (item in clientesRegistrados){
            val cliente = item.value
            array.add(cliente)
        }

        var listAdapter = ArrayAdapter(this,android.R.layout.simple_list_item_1, array)
        listaClientes.adapter = listAdapter

    }


    private fun ingresarCliente(){
        val intent = Intent(this,IngresarCliente::class.java)
